...
function ReporterTrx(runner, options) {
    ...
    runner.on('end', () => {
        const testResults = {
            stats: self.stats || { start: new Date(), end: new Date() },
            tests: [...tests.values()],
        };
...
function getFilename(reporterOptions) {
    let fileName = reporterOptions.output || process.env.MOCHA_REPORTER_FILE;
    if (fileName && reporterOptions.appendTimestamp === true) {
        ...
        if (lastIndex === -1) {
            fileName = `${fileName}_${isoDate}`;
        } else {
            fileName = `${fileName.substring(0, lastIndex)}_${isoDate}${fileName.substring(lastIndex, fileName.length)}`;
        }
    }
